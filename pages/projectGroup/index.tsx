...
import { useCreateProjectGroups, useDeleteProjectGroups, useEditProjectGroups, useProjectGroupsList } from '@/hooks/projectGroup';
import { ProjectGroups } from '@/types/projectGroup';
...
const ProjectGroup = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Project Groups'));
    });
    const [{ data, error, loading }, refetch] = useProjectGroupsList({});
    const [{ loading: loadingCreate, error: errorCreate }, doCreate] = useCreateProjectGroups();
    const [{ loading: loadingEdit, error: errorEdit }, doEdit] = useEditProjectGroups();
    const [{ loading: loadingDelete, error: errorDelete }, doDelete] = useDeleteProjectGroups();
    ...
    const [initialRecords, setInitialRecords] = useState<ProjectGroups[]>([]);
    ...
    const [params, setParams] = useState<ProjectGroups>(JSON.parse(JSON.stringify(defaultParams)));
    ...
    const deleteUser = (user: any = null) => {
        ...
        doDelete({
            url: `/project-groups/${user.id}`,
        }).then((res) => {
            console.log('res', res);
        });
        showMessage('Project Groups has been deleted successfully.');
    };
    ...
    const saveUser = () => {
        ...
        showMessage('Project Groups has been saved successfully.');
    ...
    const handleClick = useCallback(
        (data: any) => {
            ...
            if (currentTime - lastClickTime < doubleClickThreshold) {
                ...
                window.localStorage.setItem('projectGroups', JSON.stringify(data));
                router.push(`/projectGroup/${data.id}`);
            } else {
    ...
    return (
        <div>
            <Breadcrumbs
                listArr={[
                    { name: 'Home', link: '/' },
                    { name: 'Project Groups', link: '/projectGroup', active: true },
                ]}
            />
